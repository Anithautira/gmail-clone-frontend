...
function TrashEmails() {
    ...
    function moveToInboxClick(email) {
        ...
        axios.put(`https://backend-gmail-cloneport.onrender.com/updateEmailInfo/${email._id}`, sendingEmailToInbox)
            .then((response) => {
    ...
    function emptyTrashNow() {
        if (trashEmails.length !== 0) {
            axios.delete(`https://backend-gmail-cloneport.onrender.com/deleteAllTrashEmails/${userEmail}`)
                .then((response) => {
