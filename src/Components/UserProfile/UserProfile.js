...
function UserProfile() {
    ...
    async function submitClick() {
        ...
        if (nameError.innerText === "" && passwordError.innerText === "" && securityCodeError.innerText === "") {
            ...
            await axios.put(`http://localhost:6000/updateUser?email=${userData.email}&securityCode=${security}`, newPassword)
                .then((res) => {
                    ...
                    async function getUserData() {
                        let passwordInput = '';
                        await axios.get(`http://localhost:6000/login?email=${email.value}&password=${passwordInput}`)
                            .then((res) => {
    ...
    async function deleteAccount(userId) {
        ...
        if (confirmation) {
            await axios.delete(`http://localhost:6000/deleteUser/${userId}`)
                .then((res) => {
