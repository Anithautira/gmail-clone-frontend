...
function Login() {
    ...
    function loginClick() {
        ...
        if (emailError.innerText === "" && passwordError.innerText === "") {
            axios.get(`http://localhost:6000/login?email=${email.value}&password=${password.value}`)
                .then((res) => {
                    if (res.data.message === "Login Successful") {
                        async function getAllRecipes() {
                            await axios.get('http://localhost:6000/allRecipes')
                                .then((res) => {
                                    sessionStorage.setItem('allRecipes', JSON.stringify(res.data))
                                })
                                .catch((error) => {
    ...
    const response = async (res) => {
        ...
        axios.get(`http://localhost:6000/login?email=${userObject.email}&password=${password}`)
            .then((res) => {
                if (res.data.message === "Login Successful") {
                    axios.get('http://localhost:6000/allRecipes')
                        .then((res) => {
                            sessionStorage.setItem('allRecipes', JSON.stringify(res.data))
                        })
                        .catch((error) => {
